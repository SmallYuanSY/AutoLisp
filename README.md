...
| TXTEXPORT.lsp | 文字配對輸出工具 | 將文字按行配對並追加到 TXT 檔案，方便資料匯出 |
...
| 指令名稱 | 功能說明 | 操作說明 |
| :------- | :------- | :------- |
| OFF.lsp | 線段外框擴張整合工具 | 選擇 Polyline 或 Line 模式，呼叫對應的 OFFA 或 OFFT 功能 |
| OFFA.lsp | 線段外框擴張（拆解版） | 將選取的線段或 Polyline 拆開後，分別向外建立外框 |
| OFFT.lsp | Polyline 整體擴張外框 | 專門對封閉 Polyline，一次整體向外建立外框（不拆開） |
| DIVCPY.lsp | 線段等分複製工具 | 將直線等分成指定段數，可選擇是否刪除原始線段 |
| SWAPBLOCK.lsp | 框選範圍圖塊替換 | 框選範圍內的圖塊，批量替換成指定的目標圖塊 |
| FINDNET.lsp | 網絡線段搜尋工具 | 從選定線段開始，自動尋找所有相連的線段網絡 |
...
| AREASUM.lsp | 面積計算加總工具 | 選取封閉多段線、圓形等物件，自動計算並加總面積 |
| READAREA.lsp | 面積讀取標註工具（RA） | 選取物件計算面積，支援單位轉換並可標註到圖面 |
| AUTONUMBER.lsp | 自動編號工具 | 選取物件後自動在旁邊建立遞增編號文字 |
...
| CALC.lsp | 文字數學表達式計算器 | 選取包含數學表達式的文字，自動計算並顯示結果 |
| STMC.lsp | 文字對齊中央工具 | 將選取的文字物件統一設定為中央對齊 |
| BD.lsp | 文字方格排列工具 | 將文字按順序排列到指定的方格中 |
| AF.lsp | 自動折線圓角工具 | 框選物件自動執行圓角指令，以最長兩條邊做折線 |
| SPLITBLOCK.lsp | 圖塊外框分割工具 | 選取圖塊或外參，建立其邊界矩形 |
| GTABLE.lsp | 表格 ID 查詢工具 | 選取表格物件，顯示其 Object ID |
| AT.lsp | 文字排序輔助工具 | 提供文字排序相關的輔助函數 |
| Mod-U.lsp | U 型多段線修改工具 | 選取 U 型 polyline 進行幾何修改 |
| TMD.lsp | 文字中點移動工具 | 將文字移動到兩個物件的中點位置 |
| CheckCross.lsp | 線段交點檢查工具 | 檢查主線與其他線段的交點情況 |
| tozero.lsp | 物件移至原點工具 | 將選取物件的基準點移動到座標原點 (0,0,0) |
| Cont.lsp | 管線圖塊計數工具 | 計算管線上的圖塊數量統計 |
